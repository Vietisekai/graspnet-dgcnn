...
class GraspNetDataset(Dataset):
    def __init__(self, root, valid_obj_idxs, grasp_labels, camera='kinect', split='train', num_points=20000,
        ...
        if split == 'train':
            self.sceneIds = list( range(90,95) )
        elif split == 'test':
            self.sceneIds = list( range(95,100) )
        elif split == 'test_seen':
            self.sceneIds = list( range(95,97) )
        elif split == 'test_similar':
            self.sceneIds = list( range(97,99) )
        elif split == 'test_novel':
            self.sceneIds = list( range(99,100) )
